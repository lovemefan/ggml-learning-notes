...
ggml/include
└── ggml
    ├── ggml-alloc.h // 用于分配tensor,计算图的内存
    ├── ggml-backend.h // 适配多种后端
    └── ggml.h
```
...
struct ggml_tensor {
        ...
        enum ggml_backend backend; // 设备后端，可以是metal cpu gpu 

        int     n_dims; // 维度， 例如三维矩阵，为3
  
  			// 每个维度的大小，相当于shape 或size， 
        // 通常是反着存， 例如矩阵[[[0,4,8], [1,5,9]], [[1,5,9], [3,7,11]]] 
        // 该矩阵的numpy数组的shape为(2,2,3) 的 n_dims为[2,2,3] 
        ...
        // nb[i] = nb[i-1] * ne[i-1]
        // padding 为了内存地址对齐
        // nb数组就很牛逼了，它记录了每一个维度的地址偏移量，用于计算高维矩阵元素的地址，
        // 以及reshape、transpose、permute等操作，只需要交换nb数组即可
        // 举个例子：
  			
        // 令矩阵A = [[[0,4,8], [2,6,10]], [[1,5,9], [3,7,11]]] ， 其中A维度为(2,2,3)的一个三维矩阵
        // 假设 ggml_type为int8，只占一个字节。 其中 n_dims = 3 , ne = [3,2,2]。
        // 假设padding=0， 则 nb=[1， 3， 6， 12]
        // 那么 A[1][1][2] -> *(A + 1*1 + 1*2 + 2*4)  = *(A + 11)
        // A[0][1][1] -> *(A + 0*1 + 1*2 + 1*4) = *(A + 4)
